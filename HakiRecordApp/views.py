...
def generate_case_pdf(request, pk):
    ...
    #  Logo
    logo_path = os.path.join(settings.BASE_DIR, 'HakiRecordApp', 'static','assets', 'img', 'logo1.png')
    ...
    if statement.incident_evidence:
        ...
        if evidence_path.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            p.drawString(x_margin, height - 140, f"File Path: {evidence_path}")
        else:
            p.setFont("Helvetica", 11)
...
def recent_actions(request):
    ...
    for action in actions:
        try:
            parsed = json.loads(action.change_message)
            if isinstance(parsed, dict) and "changed_fields" in parsed:
                action.detailed_changes = parsed["changed_fields"]
            else:
                action.detailed_changes = {}
        except Exception:
            action.detailed_changes = {}
